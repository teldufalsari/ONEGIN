 ...
 * @return - Pointer if file was opened successfully;
...
int main ()
{
    ...
    qsort(index, n_str, sizeof(char*), (int (*)(const void *, const void *)) compare_head);
    dump("Sorted by heads.txt", n_str, index);

    qsort(index, n_str, sizeof(char*), (int (*)(const void *, const void *)) compare_end);
    dump("Sorted by endings.txt", n_str, index);
...
char* import (size_t* size, char* text)
{
    ...
    for (int k = 0; k < MAXTRIES; k++)
    {
        ...
        printf("Unable to open the file or the file is empty. Try again\n");
    }
